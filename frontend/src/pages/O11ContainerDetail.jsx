...
export default function O11ContainerDetail() {
  ...
  const handleRedeploy = async () => {
    ...
    if (!ok) return;
    setRedeploying(true);
    setActionProgress({ action: "redeploy", target: name });
    try {
      ...
      setActionProgress({ action: "redeploy", target: name, finished: true });
      setEditingConfig(false);
      fetchContainer();
      refreshO11Containers();
    } catch (err) {
      const msg = err.response?.data?.detail || "Failed to redeploy container";
      toast.error(msg);
      setActionProgress({
        action: "redeploy",
        target: name,
            ...
            <div className="space-y-6">
              ...
              {(() => {
                ...
                if (!vpnParentName && proxyNets.length > 0) {
                  const proxyEntries = activeProxyEntries.slice(0, 6);
                  return (
                    <div>
                      <h3 className="text-sm font-semibold text-vpn-muted uppercase tracking-wider mb-3">
                ...
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-400">
                      ...
                      be stopped and removed, then recreated with the new
                      network mode. All data in non-persistent volumes will be
                      lost.
                    </p>
                  </div>
                </div>
                ...
                <div className="bg-amber-500/10 border border-amber-500/30 rounded-lg p-3">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-amber-400">
                      ...
                      recreate it with the new configuration. Non-persistent
                      data will be lost.
                    </p>
                  </div>
                </div>
