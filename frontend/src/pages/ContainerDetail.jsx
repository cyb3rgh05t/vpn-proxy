...
export default function ContainerDetail() {
  ...
  const handleAction = async (action) => {
    ...
    setActionProgress({ action, target: container?.name || id });
    try {
  ...
  const handleRedeploy = async () => {
    ...
    if (!ok) return;
    setRedeploying(true);
    setActionProgress({ action: "redeploy", target: container.name });
    try {
      const payload = {
        config: {
          ...editConfig,
              ...
              <div>
                ...
                {container.config?.SHADOWSOCKS?.toLowerCase() === "on" &&
                  (() => {
                    ...
                    const ssExtPort = ssMapping
                      ? parseInt(ssMapping.host)
                      : null;
                    ...
                    <div>
                      <div className="flex items-center gap-2 mb-1">
                        ...
                        <button
                          type="button"
                          onClick={() =>
                            setEditSocks5Enabled(!editSocks5Enabled)
                          }
                          className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                            editSocks5Enabled
                              ? "bg-vpn-primary"
